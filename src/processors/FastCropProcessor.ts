...
import SmartCropWorker from '../workers/smartCrop.worker.ts?worker';

/**
 * FastCropProcessor - Smart cropping without background removal
 *
 * Strategy Pattern implementation using Web Worker for off-thread processing.
 * Uses edge detection to crop images to clothing boundaries.
 *
 * Performance: ~200-500ms per image, no UI freeze
 */
export class FastCropProcessor implements ImageProcessor {
  public readonly name = 'Fast Crop';
  public readonly description = 'Smart cropping without background removal';
  public readonly avgTimePerImage = '200-500ms';
  ...
  private initWorker(): Worker {
    if (!this.worker) {
      this.worker = new SmartCropWorker();
    }
    return this.worker;
  }

  /**
   * Process image using smart crop worker
   *
